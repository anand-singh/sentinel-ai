...
/**
 * Unit tests for ActionExecutor agent.
 ...
 */
@DisplayName("ActionExecutor Agent Tests")
class ActionExecutorTest {
    ...
    void setUp() {
        ...
        agent = ActionExecutor.initAgent();
    ...
    void testAgentInitialization() {
        ...
        assertThat(llmAgent.name()).isEqualTo("ActionExecutor");
        assertThat(llmAgent.description()).contains("Executes approved actions");
    ...
    void testRootAgentInitialization() {
        assertNotNull(ActionExecutor.ROOT_AGENT, "ROOT_AGENT should be initialized");
        assertInstanceOf(LlmAgent.class, ActionExecutor.ROOT_AGENT, 
    ...
    void testOutputKey() {
        ...
        assertThat(outputKey.get()).isEqualTo("action_execution_result");
    }
    ...
    void testInstructionContent() {
        ...
        assertThat(instructionStr).containsAnyOf("Action Executor", "action", "executor");
    }
    ...
    @Test
    @DisplayName("Should have description mentioning approved actions")
    void testAgentDescription() {
        LlmAgent llmAgent = (LlmAgent) agent;
        assertThat(llmAgent.description())
                .contains("approved actions")
                .contains("final risk decisions");
    }

    @Test
    @DisplayName("Agent should be accessible via ROOT_AGENT for ADK UI")
    void testRootAgentAccessibility() {
        assertNotNull(ActionExecutor.ROOT_AGENT, "ROOT_AGENT should be accessible");
        assertSame(ActionExecutor.ROOT_AGENT.getClass(), agent.getClass(), 
    ...
    void testAgentName() {
        LlmAgent llmAgent = (LlmAgent) agent;
        assertEquals("ActionExecutor", llmAgent.name());
    }
    ...
    void testInitializationNoExceptions() {
        assertDoesNotThrow(() -> {
            BaseAgent newAgent = ActionExecutor.initAgent();
    ...
    void testAgentType() {
        assertTrue(agent instanceof LlmAgent, "Agent should be instance of LlmAgent");
        assertTrue(ActionExecutor.ROOT_AGENT instanceof LlmAgent, 
    ...
    void testToolCount() {
        ...
        // ActionExecutor should have 5 tools:
        // freezeTransaction, notifySecurityTeam, createCaseReport, 
        // requestStepUpAuth, escalateToHuman
    ...
    void testDescriptionKeywords() {
        ...
        assertTrue(description.contains("action") || description.contains("execute"), 
                "Description should mention actions or execution");
    }

    @Test
    @DisplayName("Should initialize agent multiple times independently")
    void testMultipleInitializations() {
        BaseAgent agent1 = ActionExecutor.initAgent();
        BaseAgent agent2 = ActionExecutor.initAgent();
    ...
    void testRootAgentType() {
        ...
        LlmAgent rootAgent = (LlmAgent) ActionExecutor.ROOT_AGENT;
